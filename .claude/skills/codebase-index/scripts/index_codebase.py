...
class CodebaseIndexer:
    ...
    def _process_component_file(self, file_path: Path):
        ...
        try:
            ...
            # Resolve component imports to their file paths
            uses = []
            for imp in imports:
                if imp["is_component"]:
                    resolved = self._resolve_import_to_component_key(imp["source"], file_path)
                    if resolved:
                        uses.append(resolved)
                    else:
                        uses.append(imp["name"])  # fallback to name
            ...
            component_info = {
                ...
                "uses": list(dict.fromkeys(uses)),  # deduplicate, preserve order
    ...
    def _is_component_import(self, source: str) -> bool:
        ...
        if any(source.endswith(ext) for ext in component_extensions) or '/components/' in source:
            return True
        # Relative imports that aren't utilities or schemas are likely sibling components
        if source.startswith('.') and not self._is_utility_import(source) and not self._is_schema_import(source):
            return True
        return False

    def _resolve_import_to_component_key(self, source: str, importing_file: Path) -> Optional[str]:
        """Resolve a relative import source to a component key (relative path from project root)"""
        ...
        for ext in self.config["extensions"]:
            ...
            if candidate.exists():
                try:
                    ...
                    pass
        # Try as-is (source already has extension)
